...
class BuilderGenerator(
    ...
    override fun process(resolver: Resolver): List<KSAnnotated> {
        val annotated =
            resolver
                .getSymbolsWithAnnotation<GenerateBuilder>()
    ...
    private fun generateBuilderClass(decl: KSClassDeclaration) {
        ...
        FileSpec
            .builder(classDescriptor.packageName, classDescriptor.builderName)
            .toastIndent()
            ...
            .writeTo(codeGenerator, Dependencies(aggregating = false, decl.containingFile!!))
    }

    private fun PropertyDescriptor.makeSpec() =
        PropertySpec
            .builder(name, builderType)
            .mutable()
    ...
    private fun SimpleClassDescriptor.makeBuildMethod(): FunSpec {
        ...
        val constructorParams =
            properties.joinToString {
                "${it.name} = " +
                    if (it.type.isNullable || it is ContainerPropertyDescriptor) {
                        it.name
                    } else {
                        """
    ...
    private inline fun <reified T : Annotation> Resolver.getSymbolsWithAnnotation() = getSymbolsWithAnnotation(T::class.qualifiedName!!)
}
